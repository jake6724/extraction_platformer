...
func enter(_previous_state_path: String, _data := {}) -> void:
	enemy.rotate_on_y(enemy._current_patrol_direction)
...
func patrol(delta: float) -> void:
	if not enemy.is_on_floor():
		enemy.move_and_fall(delta, enemy.patrol_speed, enemy._current_patrol_direction, enemy.acceleration)
		return 
	...
	# Turn around
		...
		enemy._current_patrol_direction *= -1
		enemy.rotate_on_y(enemy._current_patrol_direction)
		return
